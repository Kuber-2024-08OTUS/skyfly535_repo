...
```
3. Cоздан манифест `namespace.yaml` для namespace с именем `homework`.
...
4. Создан манифеста `pod.yaml`, описывающий поднимаемый Pod и удовлетворяющий требованиям ДЗ.
...
5. Для отработки сетевого взаимодействия поднятого Pod и для получения доступа к нему через браузер на хостовой системе, написан манифеста для `Service` с типом `LoadBalancer`.
